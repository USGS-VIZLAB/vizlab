...
#'
#' @export
makeMakefiles <- function(blocks=c('fetch','process','visualize', 'publish'), display=FALSE) {
  makefiles <- c(
    list(makefile=makeTopMakefile()),
    lapply(setNames(blocks, paste0(blocks, '.make')), function(block) {
      makeBlockMakefile(block, outfile=paste0('vizlab/make/', block, '.make'))
    })
  )
  if(display) {
...
#'
#' @export
makeTopMakefile <- function(outfile='Makefile') {
  makefile <- paste(
    paste0('# Makefile for overall visualization project\n# Do not edit (autogenerated)'),
    makeMakeMacros(),
    makeMakeRunmakeRules(),
    makeCleanRule(),
    sep='\n\n')
...
#'
#' Each makefile gets a rule to run it and a rule to make the makefile
#'
#' @keywords internal
makeMakeRunmakeRules <- function() {
...
#' makefiles
#'
#' @param block character name of the block in the viz.yaml for which to make a
#'   makefile
#' @param outfile filename where the makefile should be written
#'
#' @export
makeBlockMakefile <- function(block=c('fetch','process','visualize','publish'), outfile) {
  block <- match.arg(block)
  makefile <- paste(
    paste0('# Makefile for ', block, ' phase of analysis\n# Do not edit (autogenerated from viz.yaml)'),
    makeMakeMacros(),
    makeMakeBlockRules(block),
    sep='\n\n')
...
#'
#' @export
makeMakeBlockRules <- function(block=c('fetch','process','visualize','publish')) {
  block <- match.arg(block)
...
#' \code{makeMakeItem.publish}: Make makefile rules for an item in the
#' publish block of viz.yaml
#'
#' @rdname makeMakeItem
#' @export
makeMakeItem.publish <- function(item.info, ...) {
  makeMakeRulePair(item.info, block='publish', concat=TRUE)
}
...
#' @rdname makeMakeItem
#' @export
makeMakeItem.makefile <- function(item.info, ...) {
  squote <- function(x) paste0("'", x, "'")
  rules <- list()
  rules$call.make <- makeMakeShellRule(
    target=item.info$id,
    depends=item.info$location,
    cmd=paste0('make -f ', item.info$location))
  rules$file.make <- makeMakeBatchRule(
    target=item.info$location,
    depends=item.info$depfiles,
    fun='makeBlockMakefile',
...
#' @keywords internal
makeMakeShellRule <- function(target, depends=c(), cmd) {
  # produce the final character string
  paste(c(
    makeMakeEmptyRule(target, depends),
    paste0('\t', cmd)),
