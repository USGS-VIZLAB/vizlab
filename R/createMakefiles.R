...
createShortcuts <- function(){
  blocks <- c('content','fetch','process','visualize','publish')
...
#' Create call to the Content make file
#'
#' This includes an internal step "content" which is needed for triggering
#' a rebuild if the viz.yaml file is changed.
#'
#' @keywords internal
createContentMakefileCall <- function(){
  paste('content: vizlab/make/content.make\n\tmake -f vizlab/make/content.make\nvizlab/make/content.make: viz.yaml ~/.vizlab/profile.yaml')
}
...
#' Create the content make file
#'
#' Content make file to re-build yaml files
#'
#' @keywords internal
createContentMakefile <- function(){
  
  makefile <- paste(
    paste0('# Makefile for content phase of analysis\n# Do not edit (autogenerated from viz.yaml)'),
    createMakeMacros(),
    createMakeContentBlockRules(),
    sep='\n\n')
  createMakeDirs(makefile)
  writeLines(makefile, con="vizlab/make/content.make")
  invisible(makefile)
}
...
#'
#' @keywords internal
createMakeContentBlockRules <- function(){

  paste(c(
    "all:generate-content",
    "generate-content: viz.yaml",
    sprintf('\texport R_LIBS_USER=$(RLIBSUSER);\\'),
    sprintf("\t${RSCRIPT}  -e 'library(vizlab); updateContent()'")),
    collapse='\n')
}
...
#'
#' @export
updateContent <- function(block=c('fetch','process','visualize','publish')){
  content.info <- getContentInfos(block=block)
  for(id in as.character(sapply(content.info, function(x)x$id))){
    updateContentInfoFile(id)
  }
}

#' updateContentInfoFile
#' 
...
#'   
#' @export
updateContentInfoFile <- function(viz.id){
