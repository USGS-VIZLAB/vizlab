...
#'
#' @export
createMakefiles <- function(blocks=c('fetch','process','visualize', 'publish'), profilepath = ".", display=FALSE) {
  createMakeRunmakeRules(profilepath)
  makefiles <- c(
    list(makefile=createTopMakefile(profilepath=profilepath)),
...
#'
#' @export
createTopMakefile <- function(outfile='Makefile', profilepath) {
  makefile <- paste(
    paste0('# Makefile for overall visualization project\n# Do not edit (autogenerated)'),
    createMakeMacros(profilepath),
    createMakeRunmakeRules(profilepath),
    createCleanRule(),
    sep='\n\n')
...
#' @param profilepath filepath for profile.yaml
#' @keywords internal
createMakeRunmakeRules <- function(profilepath) {
  ...
  content.info <- lapply(setNames(blocks, paste0('top/', blocks)), function(block) {
    list(
      ...
      depfiles=c('viz.yaml', if(file.exists(profilepath)) profilepath),
      block='makefile'
    )
  })
...
#'
#' @export
createBlockMakefile <- function(block=c('fetch','process','visualize','publish'), outfile, profilepath) {
...
#' @param profilepath filepath for profile.yaml
#' @export
createMakeMacros <- function(profilepath) {
  # read user settings from profile.yaml
  profile <- getProfileInfo(profilepath)
