...
### Controlling the fetch section
  Every fetch item has a `fetcher`, either implemented by `vizlab` or within your code. There must be methods for both `fetch` and `fetchTimestamp` for every fetcher. By default, timestamps are checked (using `fetchTimestamp`) every time you build a `fetch` item with `make`. You can slow down the frequency of timestamp checks or eliminate checks altogether by placing a line in a file called preferences.yaml in your main project directory. For example:
...
timetolive:
  ...
  Cuyahoga: 6 hours
```
means that the `iris-data` item will never have its timestamp checked, while `Cuyahoga` will have its timestamp checked on any build occuring more than 6 hours after the timestamp noted in `Cuyahoga`'s timestamp file. The default (for unmentioned fetch items) is "0 days". If the timestamp is checked and has changed, the item with be re-fetched. Edits to the viz.yaml chunk for the fetch item will also trigger re-fetches, regardless of the timestamp's value or whether it has exceeded its timetolive.
