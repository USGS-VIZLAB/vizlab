...
```
wwatkins@IGSARMEWLTWDW-M:~/Documents/R/example_viz$ make
...
### Debugging
  Because all R commands are executed by the makefiles, the output does not print to the console, but can be viewed in the various `.Rout` files in `vizlab/make/log/` for each section.  The names are printed to the console when the makefile is executed, as seen above.
